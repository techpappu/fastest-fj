        ...
        <div class="bg-brand-dark text-white text-xs py-2">
            <div class="container mx-auto px-4 flex justify-between items-center">
                ...
                <div class="flex items-center gap-4 mx-auto sm:mx-0">
                    <a href="tel:<?php echo esc_attr(get_theme_mod('fastest_fj_phone', '1-800-123-4567')); ?>"
                        class="hover:text-brand-gold transition">
                        <i class="fas fa-phone mr-1"></i>
                        <?php echo esc_html(get_theme_mod('fastest_fj_phone', '1-800-123-4567')); ?>
                    </a>
                </div>
            </div>
        </div>
        ...
        <header id="masthead" class="sticky top-0 z-50 bg-white shadow-sm">
            <div class="container mx-auto px-2 py-2">
                <div class="flex items-center justify-between gap-4">
                    <!-- Mobile Menu Button -->
                    <button id="mobileMenuBtn" class="lg:hidden text-brand-text text-xl"
                        aria-label="<?php esc_attr_e('Open menu', 'fastest_fj'); ?>">
                    ...
                    if (has_custom_logo()) {
                        ...
                        <a href="<?php echo esc_url(home_url('/')); ?>" class="flex items-center gap-2 flex-shrink-0"
                            ...
                            <div class="">
                                <h1
                                    class="font-serif lg:text-2xl text-sm md:text-xl font-bold text-brand-text leading-none">
                                    <?php bloginfo('name'); ?>
                                </h1>
                                <p class="text-[10px] tracking-widest text-brand-gold uppercase">
                    ...
                    <nav class="hidden lg:flex items-center gap-8"
                        aria-label="<?php esc_attr_e('Primary', 'fastest_fj'); ?>">
                        <?php
                        wp_nav_menu(array(
                        ...
                        <div class="relative hidden md:block">
                            <form role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>"
                                ...
                                <input type="search" name="s"
                                    placeholder="<?php esc_attr_e('Search jewelry...', 'fastest_fj'); ?>"
                                    class="pl-4 pr-10 py-2 border border-gray-200 rounded-full text-sm w-48 focus:outline-none focus:border-brand-gold transition"
                                    value="<?php echo get_search_query(); ?>">
                                <input type="hidden" name="post_type" value="product">
                                <button type="submit"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-brand-gold"
                                    aria-label="<?php esc_attr_e('Search', 'fastest_fj'); ?>">
                                    <i class="fas fa-search"></i>
                                </button>
                            </form>
                        </div>
                        <button class="md:hidden text-brand-text hover:text-brand-gold transition"
                            aria-label="<?php esc_attr_e('Search', 'fastest_fj'); ?>"
                            onclick="document.querySelector('.mobile-search').classList.toggle('hidden')"><i
                                class="fas fa-search text-lg"></i></button>

                        <!-- Account -->
                        <a href="<?php echo esc_url(wc_get_page_permalink('myaccount')); ?>"
                            class="text-brand-text hover:text-brand-gold transition hidden sm:block"
                            aria-label="<?php esc_attr_e('My Account', 'fastest_fj'); ?>">
                            <i class="far fa-user text-lg"></i>
                        </a>

                        <!-- Wishlist -->
                        <a href="<?php echo esc_url(get_permalink(get_page_by_path('wishlist'))); ?>"
                            class="text-brand-text hover:text-brand-gold transition relative"
                            aria-label="<?php esc_attr_e('Wishlist', 'fastest_fj'); ?>">
                            <i class="far fa-heart text-lg"></i>
                            <span
                                class="header-cart-count wishlist-count"><?php echo esc_html(fastest_fj_get_wishlist_count()); ?></span>
                        </a>

                        <!-- Cart -->
                        <a href="<?php echo esc_url(wc_get_cart_url()); ?>"
                            class="text-brand-text hover:text-brand-gold transition relative"
                            aria-label="<?php esc_attr_e('Cart', 'fastest_fj'); ?>">
                            <i class="fas fa-shopping-bag text-lg"></i>
                            <span
                                class="header-cart-count"><?php echo esc_html(WC()->cart->get_cart_contents_count()); ?></span>
                        </a>
                    </div>
                </div>
                ...
                <div class="mobile-search hidden mt-4 md:hidden">
                    <form role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>">
                        <input type="search" name="s"
                            placeholder="<?php esc_attr_e('Search jewelry...', 'fastest_fj'); ?>"
                            class="w-full px-4 py-2 border border-gray-200 rounded-full text-sm focus:outline-none focus:border-brand-gold transition">
                        <input type="hidden" name="post_type" value="product">
                    </form>
                </div>
            </div>
            ...
            <!-- Mobile Menu -->
            <div id="mobileMenu"
                class="mobile-menu fixed inset-y-0 left-0 z-50 w-80 bg-white shadow-xl lg:hidden overflow-y-auto">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-8">
                        <h2 class="font-serif text-xl font-bold"><?php esc_html_e('Menu', 'fastest_fj'); ?></h2>
                        <button id="closeMobileMenu" class="text-2xl text-brand-text"
                            aria-label="<?php esc_attr_e('Close menu', 'fastest_fj'); ?>"><i
                    ...
                    <nav aria-label="<?php esc_attr_e('Mobile', 'fastest_fj'); ?>">
                        ...
                        <div class="border-t border-gray-100 pt-4 mt-4">
                            <a href="<?php echo esc_url(wc_get_page_permalink('myaccount')); ?>"
                                class="text-lg py-2 flex items-center gap-2"><i class="far fa-user"></i>
                                <?php esc_html_e('My Account', 'fastest_fj'); ?></a>
                        </div>
                    </nav>
                </div>
            </div>
